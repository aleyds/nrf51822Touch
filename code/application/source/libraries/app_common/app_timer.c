...
#define APP_TIMER_DEBUG				"APP_TIMER"
...
static void timer_timeouts_check(void)
{
    ...
    if (m_timer_id_head != TIMER_NULL)
    {
        ...
        while (timer_id != TIMER_NULL)
        {
			...
			//WYPrintf(APP_TIMER_DEBUG, LEVEL_DEBUG, "ticks_elapsed:%d  last:%d  count:%d timer:%d",ticks_elapsed,m_ticks_latest,rtc1_counter_get(),p_timer->ticks_to_expire);
