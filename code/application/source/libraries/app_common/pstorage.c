...
static uint32_t cmd_process(void)
{
    ...
    switch (p_cmd->op_code)
    {
        ...
        case PSTORAGE_UPDATE_OP_CODE:
        {
            ...
            uint32_t head_word_size = (storage_addr + p_cmd->offset -(page_number * PSTORAGE_FLASH_PAGE_SIZE)) / sizeof(uint32_t);

            uint32_t tail_word_size = (((page_number + 1) * PSTORAGE_FLASH_PAGE_SIZE) -(storage_addr + p_cmd->offset + p_cmd->size)) / sizeof(uint32_t);
