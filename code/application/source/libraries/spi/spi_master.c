...
_static P_EVENT_HANDLER                 pIraEventHandler = 0;
...
{
	//WYPrintf("spi",LEVEL_DEBUG,"spi handle in");
...
{
    APP_ERROR_CHECK_BOOL(p_spi_instance != NULL);
    if(p_spi_instance->tx_enable == 1)
    {
    	 uint8_t send_txt = ((p_spi_instance->p_tx_buffer != NULL) &&
                                      (p_spi_instance->tx_index < p_spi_instance->tx_length)) ?
                                     p_spi_instance->p_tx_buffer[p_spi_instance->tx_index] :
                                     SPI_DEFAULT_TX_BYTE;
   		 p_spi_instance->p_nrf_spi->TXD = send_txt;
...
static __INLINE void spi_master_send_recv_irq(volatile spi_master_instance_t * const p_spi_instance)
{
    ...
    //WYPrintf(SPI_MASTER, LEVEL_DEBUG, "RXD :%d  TXD:%d",rx_byte,p_spi_instance->p_nrf_spi->TXD);
    if ((p_spi_instance->p_rx_buffer != NULL) && (p_spi_instance->rx_enable == 1)&&
    ...
    if ((p_spi_instance->tx_index < p_spi_instance->max_length) && (p_spi_instance->tx_enable == 1))
    {
    	...
    	uint8_t send_txt = ((p_spi_instance->p_tx_buffer != NULL) &&
                                         ...
                                         p_spi_instance->p_tx_buffer[p_spi_instance->tx_index] :
                                         SPI_DEFAULT_TX_BYTE;
       // WYPrintf(SPI_MASTER, LEVEL_DEBUG, "TXD :%d ",send_txt);
       //wy_tools_op()->_delay_ms(50);
        p_spi_instance->p_nrf_spi->TXD = send_txt;
        (p_spi_instance->tx_index)++;
    }
...
uint32_t spi_master_open(const spi_master_hw_instance_t    spi_master_hw_instance,
                         spi_master_config_t const * const p_spi_master_config,uint8_t cs_trigger)
{
    #if defined(SPI_MASTER_0_ENABLE) || defined(SPI_MASTER_1_ENABLE)
...
uint8_t spi_master_occupy(const spi_master_hw_instance_t spi_master_hw_instance)
{
	volatile spi_master_instance_t * p_spi_instance = spi_master_get_instance(
        spi_master_hw_instance);
        ...
        spi_master_init_hw_instance(NRF_SPI0, SPI0_TWI0_IRQn, p_spi_instance, 0);
    //APP_ERROR_CHECK_BOOL(p_spi_instance != NULL);
    if((p_spi_instance->pin_slave_select != (uint8_t)0xFF) 
        && (p_spi_instance->p_nrf_spi->ENABLE == (SPI_ENABLE_ENABLE_Enabled << SPI_ENABLE_ENABLE_Pos)))
    	{
    		return 1;
    	}
...
void spi_master_chip_cs(const spi_master_hw_instance_t spi_master_hw_instance, uint8_t on)
{
    ...
    else
    {
    	...
    	//WYPrintf(SPI_MASTER, LEVEL_DEBUG, "spi_master_chip_cs");
    }
}

void spi_master_close(const spi_master_hw_instance_t spi_master_hw_instance,uint8_t cs_trigger)
{
...
void cc3100_irq_callback(void)
{
	if(pIraEventHandler != 0 )
	{
		pIraEventHandler(0);
	}
}
...
uint32_t spi_master_send_recv(const spi_master_hw_instance_t spi_master_hw_instance,
                              ...
                              uint8_t * const p_rx_buf, const uint16_t rx_buf_len,uint8_t cs_trigger)
{
    ...
    else
    {
        ...
        uint32_t ret_code = sd_nvic_EnableIRQ(p_spi_instance->irq_type);
        //APP_ERROR_CHECK(sd_nvic_EnableIRQ(p_spi_instance->irq_type));
